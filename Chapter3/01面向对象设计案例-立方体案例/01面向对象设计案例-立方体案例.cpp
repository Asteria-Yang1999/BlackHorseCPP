...
public:
    ...
    void getCubeS()
    {
        ...
        cout << "立方体的面积为： " << 2*m_L*m_W +2*m_L*m_H + 2*m_W*m_H << endl;
...
//    cub.getL();
//}
//为什么const刚刚不能添加？因为没办法保证成员函数里是否修改了成员属性
void func(const Cube &cub)
{
    cub.getL();
}
...
void test01()
{
    ...
    if(ret)
    {
        cout << "c1 和 c1 是相等的！" << endl;

    }
    else
    {
        cout << "c1 和 c1 是不等的！" << endl;
    }
    ...
    if(ret2)
    {
        cout << "通过成员，c1 和 c1 是相等的！" << endl;

    }
    else
    {
        cout << "通过成员，c1 和 c1 是不等的！" << endl;
    }
